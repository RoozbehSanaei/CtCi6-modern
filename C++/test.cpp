...
constexpr bool isUniqueChars(const std::string_view &str){
 ...
 /* The lambda function takes a character c as input and tries to insert 
 it into the unordered_set called char_set.
The insert function returns a pair of values: 
the first value is an iterator pointing to the inserted element 
(or the already existing element in the set), 
and the second value is a bool indicating whether the insertion was successful.
In this case, the ! operator negates the second value, 
so the expression !char_set.insert(c).second returns true
 if the insertion was not successful, which means that
  the character is already in the set (i.e., it is not unique).
Therefore, this line of code effectively checks whether
 the current character in the string is unique or not,
  based on whether it can be inserted into the set or not.
    */
